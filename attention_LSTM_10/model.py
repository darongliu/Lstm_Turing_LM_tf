...
def inference(input_x, embedding_dim, lstm_hidden_dim_1, vocab_size,
    ...
    with tf.variable_scope('recurrent_layer1') as vs:
        ...
        common_var = common_var + lstm1_variables
    ...
    with tf.variable_scope('attention_layer'):
        ...
        ten_var.append(W_key)
        ten_var.append(b_key)
    ...
    return logits, pretrain_list, output_linear_list, common_var, ten_var
...
def training(loss, learning_rate, grad_norm, common_var, ten_var) :
    """
    args:
        loss
    ...
    """
    tvars_common = common_var
    tvars_ten    = ten_var
    grads_common, _ = tf.clip_by_global_norm(tf.gradients(loss, tvars_common), grad_norm)
    grads_ten, _    = tf.clip_by_global_norm(tf.gradients(loss, tvars_ten), grad_norm)
    optimizer_common = tf.train.GradientDescentOptimizer(learning_rate)
    optimizer_ten = tf.train.GradientDescentOptimizer(10*learning_rate)
    train_op_common = optimizer_common.apply_gradients(zip(grads_common, tvars_common))
    train_op_ten = optimizer_ten.apply_gradients(zip(grads_ten, tvars_ten))
    train_op = tf.group(train_op_common, train_op_ten)
