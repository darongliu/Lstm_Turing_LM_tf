...
def inference(input_x, embedding_dim, lstm_hidden_dim_1, vocab_size,
    ...
    with tf.variable_scope('merge_layer'):
        ...
        b_project = tf.get_variable('b_project', [lstm_hidden_dim_1], initializer=tf.constant_initializer(0.0))
        alpha     = tf.get_variable('alpha', [], initializer=tf.constant_initializer(0.5))

        temp = tf.reshape(att_outputs, [-1,lstm_hidden_dim_1])      
    ...
    with tf.variable_scope('output_lstm1_att_linear'):
        ...
        recurrent_outputs = tf.reshape(att_lstm_outputs,[-1,lstm_hidden_dim_1])
