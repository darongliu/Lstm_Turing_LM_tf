...
def train(args):
    ...
    with tf.variable_scope('model',initializer=default_initializer):
        logits, pretrain_list, output_linear_list, entropy = model.inference(
    ...
    total_label_loss, loss = model.loss(logits=logits, labels=labels_ph, entropy=entropy, entropy_reg=args.entropy_reg)
